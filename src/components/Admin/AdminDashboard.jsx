...
import { motion } from 'framer-motion'
import { Link } from 'react-router-dom'
import { FaUsers, FaUserMd, FaCalendarAlt, FaRupeeSign, FaChartLine, FaChevronRight, FaFlask, FaXRay } from 'react-icons/fa'
import adminService from '../../services/adminService'
import LoadingSpinner from '../Common/LoadingSpinner'
import { formatCurrency } from '../../utils/helpers'
import toast from 'react-hot-toast'
...
const AdminDashboard = () => {
  ...
  // ✅ NEW: Lab & Radiology Cards for Quick Actions
  const managementCards = [
    { icon: <FaFlask />, label: 'Lab Tests', desc: 'Manage all lab tests', color: 'from-purple-500 to-indigo-600', link: '/admin/lab-tests' },
    { icon: <FaXRay />, label: 'Radiology Tests', desc: 'Manage all radiology tests', color: 'from-indigo-500 to-purple-600', link: '/admin/radiology-tests' },
  ]
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Lab & Radiology Management</h2>
            <div className="space-y-3">
              {managementCards.map((card, idx) => (
                <Link to={card.link} key={idx}>
                  <motion.div
                    whileHover={{ x: 5 }}
                    className={`bg-gradient-to-r ${card.color} p-4 rounded-xl text-white flex items-center justify-between cursor-pointer`}
                  >
